...
int main()
{
    ...
    while(!infile.eof()){
       ...
       getline(infile,key);
        for(int index = 0; index < key.length(); index++){
            if(int(key[index])< 65 || int(key[index]) > 122){   //check if it contains anything else except alphabets
                invalid_key = true;
                break;
            }
            else if(int(key[index]) > 90 && int(key[index]) < 97){  //check if it contains anything else except alphabets
                invalid_key = true;
                break;
            }
        }
        if(key=="" || invalid_key) continue;   //skip the invalid strings
        insert(key);                    // creating trie, inserting words till end of file
    }
    ...
    for(unsigned int counter=0; counter < SEARCH_RESULTS.size(); counter++)
        cout << SEARCH_RESULTS[counter] << endl;
    return 0;
}
